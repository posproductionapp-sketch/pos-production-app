...
def test_duplicate_command_is_durable_and_does_not_replace_payload() -> None:
    ...
    repository = SqlAlchemySyncRepository(session, tenant_id="tenant-a", store_id="store-a", actor_id="actor-a")

    first = repository.record_received(command_id="cmd-1", operation="sale", payload={"amount": "10.00", "currency": "THB"})
    session.commit()
    duplicate = repository.record_received(command_id="cmd-1", operation="sale", payload={"amount": "10.00", "currency": "THB"})
    ...
    with pytest.raises(SyncCommandConflict):
        repository.record_received(command_id="cmd-1", operation="sale", payload={"amount": "999.00", "currency": "THB"})
...
def test_command_scope_isolated_by_tenant_and_store() -> None:
    session = session_factory()
    tenant_a = SqlAlchemySyncRepository(session, tenant_id="tenant-a", store_id="store-a", actor_id="actor-a")
    tenant_b = SqlAlchemySyncRepository(session, tenant_id="tenant-b", store_id="store-a", actor_id="actor-a")
    store_b = SqlAlchemySyncRepository(session, tenant_id="tenant-a", store_id="store-b", actor_id="actor-a")
...
def test_stale_sending_is_recoverable_and_actor_bound() -> None:
    from datetime import datetime, timedelta, timezone

    session = session_factory()
    repository = SqlAlchemySyncRepository(session, tenant_id="tenant-a", store_id="store-a", actor_id="actor-a")
    repository.record_received(command_id="cmd-4", operation="sale", payload={"order": "o-4"})
    repository.claim("cmd-4", lease_seconds=1)
    row = repository.get("cmd-4")
    row.lease_until = datetime.now(timezone.utc) - timedelta(seconds=1)
    session.flush()
    assert repository.recover_stale_sending() == 1
    assert repository.get("cmd-4").state == "received"
    other = SqlAlchemySyncRepository(session, tenant_id="tenant-a", store_id="store-a", actor_id="actor-b")
    with pytest.raises(SyncCommandConflict):
        other.claim("cmd-4")
