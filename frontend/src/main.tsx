...
function Login({ onLogin }: { onLogin: (user: CurrentUser) => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const user = await api<CurrentUser>('/v1/me');
      onLogin(user);
    } catch (err) {
...
function Sales({ user, onLogout }: { user: CurrentUser; onLogout: () => void }) {
  ...
  const can = (...allowed: string[]) => allowed.some(role => roles.has(role));
  ...
  const nav = [
    { label: '⌂ Dashboard', allowed: ['admin', 'manager', 'cashier', 'inventory', 'auditor'] },
    ...
    { label: '♙ Customers', allowed: ['admin', 'manager', 'cashier'] },
    { label: '⌁ Reports', allowed: ['admin', 'manager', 'auditor'] },
    { label: '◉ Cash & Shift', allowed: ['admin', 'manager', 'cashier'] },
    { label: '⚙ Settings', allowed: ['admin'] },
  ];

  return <main className="dashboard"><header className="topbar"><Logo /><div className="top-actions"><span className={shift?.state === 'open' ? 'online' : 'error'}><i /> {shift?.state === 'open' ? 'Shift Open' : 'Shift Closed'}</span><span className="store" aria-label="Current store">Store {user.store_id}</span><span className="store" aria-label="Current role">{user.roles.join(', ')}</span><button onClick={onLogout}>ออกจากระบบ</button></div></header><div className="dashboard-layout"><aside className="sidebar"><nav aria-label="Primary navigation">{nav.map((item, i) => { const allowed = can(...item.allowed); const active = i === 1 && allowed; return <span className={active ? 'nav-item nav-active' : allowed ? 'nav-item' : 'nav-item nav-disabled'} aria-current={active ? 'page' : undefined} aria-disabled={!allowed} key={item.label}>{item.label}</span>; })}</nav></aside><section className="dash-content sales-view"><div className="dash-head"><div><span className="eyebrow">SALES • POS COUNTER</span><h1>หน้าขาย</h1><p>เลือกสินค้าเพื่อเริ่มรายการ</p></div><div className="stat"><span>ยอดรายการนี้</span><strong>฿{subtotal.toFixed(2)}</strong></div></div>
...
function App() {
  ...
  useEffect(() => {
    ...
    void api<CurrentUser>('/v1/me')
      .then(setUser)
      .catch(() => sessionStorage.removeItem(TOKEN_KEY))
      .finally(() => setBooting(false));
  }, []);

  if (booting) return <main className="app"><div className="loading" role="status">กำลังตรวจสอบเซสชัน…</div></main>;
  if (!user) return <Login onLogin={setUser} />;
  return <Sales user={user} onLogout={() => { sessionStorage.removeItem(TOKEN_KEY); setUser(null); }} />;
}
